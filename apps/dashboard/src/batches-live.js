...
async function checkHealth() {
  renderStatus('Connecting to Supabase…', 'Kaveri Coding');
  ...
  app.innerHTML = `
    <main class="login-shell">
      <section class="login-card">
        ...
        <p class="muted">Control batches, student access and live coding questions.</p>
...
function targetFor(assignmentId, batchId) {
  return state.targets.find((t) => t.assignment_id === assignmentId && t.batch_id === batchId);
}
...
function liveBatchRow(assignment, batch) {
  const target = targetFor(assignment.id, batch.id);
  const targeted = Boolean(target);
  const unlocked = Boolean(target?.is_unlocked);

  return `
    <div class="live-batch-row ${targeted ? 'is-targeted' : ''} ${unlocked ? 'is-unlocked' : 'is-locked'}">
      <label class="live-target-check">
        <input type="checkbox" class="target-checkbox" data-assignment="${assignment.id}" data-batch="${batch.id}" ${targeted ? 'checked' : ''} />
        <span class="check-box">${targeted ? '✓' : ''}</span>
        <span class="live-batch-name">${esc(batch.name)}</span>
      </label>
      <div class="live-state-area">
        ${!targeted
          ? '<span class="live-state not-assigned">Not assigned</span>'
          : unlocked
            ? '<span class="live-state unlocked">🔓 Unlocked</span>'
            : '<span class="live-state locked">🔒 Locked</span>'}
        ${targeted ? `
          <button class="${unlocked ? 'secondary' : 'primary'} small lock-toggle" data-assignment="${assignment.id}" data-batch="${batch.id}" data-unlock="${unlocked ? 'false' : 'true'}">
            ${unlocked ? '🔒 Lock Now' : '🔓 Unlock for Class'}
          </button>` : ''}
      </div>
    </div>`;
}
...
function assignmentCard(assignment) {
  const selected = targetBatchIds(assignment.id);
  const unlockedCount = state.targets.filter((t) => t.assignment_id === assignment.id && t.is_unlocked).length;

  return `
    <article class="delivery-card live-delivery-card">
      <div class="delivery-head">
        ...
        <div class="delivery-summary">
          <span class="delivery-mode targeted">${selected.length} target${selected.length === 1 ? '' : 's'}</span>
          <span class="live-count ${unlockedCount ? 'has-live' : ''}">${unlockedCount ? `● ${unlockedCount} live` : 'All locked'}</span>
        </div>
      </div>

      <div class="live-help-box">
        <strong>Published ≠ available.</strong>
        <span>Select the batch first. New targets start locked. Click <strong>Unlock for Class</strong> only when you want students to access the question.</span>
      </div>

      <div class="live-batch-list">
        ${state.batches.map((batch) => liveBatchRow(assignment, batch)).join('') || '<p class="empty-inline">Create a batch first.</p>'}
      </div>
    </article>`;
}
...
function renderPage() {
  ...
  const unlockedDeliveries = state.targets.filter((t) => t.is_unlocked).length;
  ...
  app.innerHTML = `
    <div class="dashboard-shell batch-shell">
      ...
      <section class="batch-metrics">
        ...
        <article class="metric-card live-metric"><span>Live now</span><strong>${unlockedDeliveries}</strong></article>
      </section>

      <section class="live-class-banner">
        <div class="live-banner-icon">🎓</div>
        <div>
          <p class="eyebrow">LIVE CLASS CONTROL</p>
          <h2>Unlock only what you are teaching now</h2>
          <p>Students see locked question titles, but question text, starter code and tests remain inaccessible until you unlock that batch.</p>
        </div>
      </section>
      ...
      <section class="batch-section">
        <div class="section-heading"><div><p class="eyebrow">Students</p><h2>Batch membership</h2></div><p class="muted">Students can join with the batch code or be added manually.</p></div>
        <div class="batch-grid">${state.batches.map(batchCard).join('') || '<section class="panel"><h3>No batches yet</h3></section>'}</div>
      </section>

      <section class="batch-section">
        <div class="section-heading"><div><p class="eyebrow">Live classroom</p><h2>Question Lock / Unlock</h2></div><p class="muted">Target → locked by default → unlock during class → lock again when class ends.</p></div>
  ...
  document.querySelectorAll('.lock-toggle').forEach((el) => el.addEventListener('click', toggleLock));
}

async function loadData() {
  renderStatus('1/5 Loading batches…', 'Live Class Manager');
  ...
  renderStatus('2/5 Loading students…', 'Live Class Manager');
  ...
  renderStatus('3/5 Loading memberships…', 'Live Class Manager');
  state.memberships = await rest('/rest/v1/batch_students?select=id,batch_id,student_id,status,enrolled_at');

  renderStatus('4/5 Loading assignments…', 'Live Class Manager');
  ...
  renderStatus('5/5 Loading live access…', 'Staff RPC');
...
async function toggleLock(event) {
  ...
  const unlock = button.dataset.unlock === 'true';
  const assignment = state.assignments.find((a) => a.id === assignmentId);
  const batch = state.batches.find((b) => b.id === batchId);

  if (unlock) {
    const ok = window.confirm(`Unlock "${assignment?.title || 'this question'}" for ${batch?.name || 'this batch'} now?\n\nStudents in that batch will be able to open the question, view visible tests and submit.`);
    if (!ok) return;
  }

  button.disabled = true;
  try {
    await rest('/rest/v1/rpc/set_coding_vscode_assignment_lock', {
      method: 'POST',
      body: {
        p_assignment_id: assignmentId,
        p_batch_id: batchId,
...
async function start() {
  try {
    ...
    const sessionResult = await withTimeout(supabase.auth.getSession(), 'Reading session');
